...
use gtk::ListStore;
use gtk::{
    CellLayoutExt, CellRendererPixbuf, CellRendererText, TreeViewColumn, TreeViewColumnExt,
...
use gtk::TreeIter;
use gtk::{TreeModelExt, TreeSelectionExt};
...
pub enum Msg {
    ...
    StopSong,
    DurationComputed(PathBuf, std::time::Duration),
    SongDuration(u64),
}
...
impl Playlist {
    ...
    fn compute_duration(&self, path: &Path) {
        ...
        self.model
            .relm
            .connect_exec_ignore_err(rx, |(path, duration)| DurationComputed(path, duration));
    }
}
